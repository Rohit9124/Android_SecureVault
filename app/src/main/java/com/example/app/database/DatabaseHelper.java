...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    private static final String COLUMN_WEBSITE = "record_website";
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        if (oldVersion < 2) {
            // Add website column for existing databases
            db.execSQL("ALTER TABLE " + TABLE_NAME + " ADD COLUMN " + COLUMN_WEBSITE + " TEXT");
        }
    }

    /**
     * Encrypts the password and adds a new entry with the given name, email,
     * (encrypted) password, and website to the database.
     *
     * @param context  The context to get the database of the application
     * @param name     The name of the entry.
     * @param email    The email of the entry.
     * @param password The password of the entry (it will be encrypted before being
     *                 inserted into the database)
     * @param website  The website URL of the entry (optional)
     */
    public static void addEntry(Context context, String name, String email, String password, String website) {
        // SQLiteDatabase db = this.getWritableDatabase(KEY_ENCRYPTION);
        SQLiteDatabase db = SQLiteDatabase.openDatabase(context.getDatabasePath(DATABASE_NAME).getAbsolutePath(),
                KEY_ENCRYPTION, null, SQLiteDatabase.OPEN_READWRITE);
     ...
     *
     * @param itemToSearch The search query used to find matching items in the
     *                     database.
     ...
     * @param website  The new value for the website column.
     */
    public void updateData(String row_id, String name, String email, String password, String website) {
        SQLiteDatabase db = this.getWritableDatabase(KEY_ENCRYPTION);
        ContentValues cv = new ContentValues();
     ...
     * @throws SQLiteException If there's an error accessing or updating the
     *                         database.
     */
    ...
    /**
     * Checks if an account with the given name and email already exists in the
     * database.
     *
     * @param name  The name of the account.
     * @param email The email of the account.
     * @return True if an account with the given name and email exists; otherwise,
     *         false.
     * @throws SQLiteException If there's an error accessing the database.
     */
    ...
    /**
     * Queries credentials from the database that match the given domain or package
     * name.
     * Used by AutofillService to find matching credentials for apps and websites.
     *
     * @param context The application context
     * @param domain  The domain (for websites) or package name (for apps) to match
     * @return A Cursor containing matching credentials, or null if none found
     * @throws SQLiteException If there's an error accessing the database.
     */
    public static Cursor getCredentialsByDomain(Context context, String domain) {
        try {
            SQLiteDatabase db = SQLiteDatabase.openDatabase(
                    context.getDatabasePath(DATABASE_NAME).getAbsolutePath(),
                    KEY_ENCRYPTION, null, SQLiteDatabase.OPEN_READONLY);

            // Query for exact match or partial match in website column
            String selection = COLUMN_WEBSITE + " LIKE ?";
            String[] selectionArgs = { "%" + domain + "%" };

            return db.query(TABLE_NAME,
                    new String[] { COLUMN_ID, COLUMN_NAME, COLUMN_EMAIL, COLUMN_PASSWORD, COLUMN_WEBSITE },
                    selection,
                    selectionArgs,
                    null, null, null);
        } catch (Exception e) {
            Log.e("DatabaseHelper", "Error querying credentials by domain", e);
            return null;
        }
    }
    ...
    public static void changeDBPassword(String newPassword, Context context) {
        ...
        SQLiteDatabase db = SQLiteDatabase.openDatabase(databasePath, KEY_ENCRYPTION, null,
    ...
    public static void exportDatabaseToJson(Context context, String passwordGotFromUser) {
        ...
        if (cursor != null && cursor.moveToFirst()) {
            do {
                ...
                try {
                    ...
                    jsonObject.put(COLUMN_PASSWORD,
                            EncryptionHelper.decrypt(cursor.getString(cursor.getColumnIndex(COLUMN_PASSWORD))));

                    // Handle website field (may be null for old entries)
                    int websiteIndex = cursor.getColumnIndex(COLUMN_WEBSITE);
                    String website = (websiteIndex != -1) ? cursor.getString(websiteIndex) : null;
                    jsonObject.put(COLUMN_WEBSITE, website);
        ...
        try {
            ...
            Toast.makeText(context, context.getString(R.string.database_successfully_exported_to) + " "
                    + Environment.DIRECTORY_DOWNLOADS, Toast.LENGTH_LONG).show();
    ...
    public static void importJsonToDatabase(Context context, Uri fileUri, String passwordGotFromUser)
            throws NoSuchAlgorithmException, InvalidKeySpecException {

        String jsonEncryptedString = readJsonFromFile(context, fileUri);
        String jsonDecryptedString = EncryptionHelper.decryptDatabase(context, jsonEncryptedString,
                passwordGotFromUser);
        ...
        try {
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                // Handle website field (may not exist in old exports)
                String website = jsonObject.optString(COLUMN_WEBSITE, null);

                if (!checkIfAccountAlreadyExist(context, name, email)) {
                    addEntry(context, name, email, password, website);
                } else {
